...
public class ChristmasDiscountPolicy implements DiscountPolicy{
    @Override
    public Price discount(Integer visitDate, December december) {
        Date lookedForDate = december.findSpecificDate(visitDate);
        if(lookedForDate.isApplicableChristmasDiscount()) {
            return new Price(MAX_CHRISTMAS_D_DAY_DISCOUNT - lookedForDate.dateFarFromChristmas() * EACH_DAY_DISCOUNT_UNIT);
        }
        return new Price(0);
    }
}
